...
class ProcessPaymentRefundCloudPayments implements ShouldQueue
{
...
//                $info = new Info($companyInn, $payment_address, $callback_url);
                ...
                $operation = $client->doOperation(
                    new OperationRequest($groupId, OperationRequest::OPERATION_SELL_REFUND, $uuid, $receipt, $info, $token)
                );
                ...
                try {
                    ...
                    Log::channel('sentry')->info('ATOL Online: чек возврата покупки успешно зарегистрирован по заказу #' . $order->id, [
                        'report' => $report,
                    ]);

                } catch (\Exception $e) {
                    Log::channel('sentry')->error('Ошибка получения отчета о регистрации чека возврата покупки', ['exception' => $e]);
                }
            } catch (\Exception $e) {
                Log::channel('sentry')->error('Ошибка регистрации чека возврата покупки', ['exception' => $e]);
            }
